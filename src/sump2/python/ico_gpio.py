...
#   self.lb.wr( self.reg_pmod_p1_cfg , [ 0x77777777 ] );# Watch SPI Traffic
    ...
    self.lb.wr( self.reg_pwm0_cfg,     [ 0x00000000 ] );
    self.lb.wr( self.reg_pwm1_cfg,     [ 0x00000000 ] );
    self.lb.wr( self.reg_pwm2_cfg,     [ 0x00000000 ] );
    self.lb.wr( self.reg_pwm0_cfg,     [ 0x01001013 ] );
    self.lb.wr( self.reg_pwm1_cfg,     [ 0x01001023 ] );
    self.lb.wr( self.reg_pwm2_cfg,     [ 0x01001033 ] );
...
#     os.system("sudo shutdown -h now");
  ...
  def not_mesa_id( self ):
    # Issue a low level MesaBus ID Request and make sure FPGA responds
    try:
      for i in range( 0,64,1 ):
        self.mb.port.wr( "FFFFFFFF\n");# Flush
      self.mb.wr( slot = 0x00, subslot = 0xF, cmd = 0xA, payload = "" );
      rts = self.mb.rd();
      if ( len( rts ) == 41 ):
        hdr = rts[0:8];
        ...
        id_snum = rts[24:32];
        id_time = rts[32:40];
        print("Device Manufacture: " + id_mfr );# 1=BML, 2=Si
        print("Device ID         : " + id_dev );# 
        print("Device SerialNum  : " + id_snum);
        print("Device Firmware   : " + id_time + " " + english_time );
      else:
        print("ERROR: MesaBus ID Request Failed");
        self.action_log( ["ERROR: MesaBus"]);
        print( len( rts ) );
        print( rts );
        sleep(1);
        return False;
