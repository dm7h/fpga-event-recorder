...
/*                   (this file has been modified for usage with TPLE by    */
/*                    Bernd.Krafft@hs-augsburg.de and                       */
/*                    Matthias.Weber@hs-augsburg.de)                        */
/*                                                                          */
/*      Supports (at least before modifications for TPLE were done) Altera  */
/*      ByteBlaster hardware download cable on Windows 95 and Windows NT    */
/*      operating systems. (A device driver is required for Windows NT.)    */
/*                                                                          */
/*      Also supports (at least before modifications for TPLE were done)    */
/*      BitBlaster hardware download cable on Windows 95, Windows NT, and   */
/*      UNIX platforms.)                                                    */
/*                                                                          */
/*      This version has been modified for usage with TPLE-Board            */
/*      using an Atmel ATMEGA32U4 as serial programmer via UART running     */
/*      LUFA USB Stack. It currently works under Linux (Ubuntu) and might   */
/*      also work under Windows (include stuff for serial interface).       */
/*      (Weber/Krafft)                                                      */
/*                                                                          */
/*      Revisions of original version:                                      */
/*        1.1  added dynamic memory allocation                              */
...
/*        2.3  added support for static memory fixed /W4 warnings           */
/*                                                                          */
/****************************************************************************/
...
#include <stdlib.h>
#include <string.h>
//#include <io.h> // fatal error: io.h: No such file or directory
#include <fcntl.h>
//#include <process.h> // fatal error: process.h: No such file or directory
...
#include <time.h>
//#include <conio.h> // fatal error: conio.h: No such file or directory
#include <ctype.h>
#include <sys/types.h>
#include <sys/stat.h>
...
int jam_jtag_io(int tms, int tdi, int read_tdo)
{
	...
	unsigned char ch_data_m = 0;
	unsigned char c = 0;
        //fprintf(stderr, "Info:  jam_jtag_io called: TMS:%3d,\tTDI:%3d,\treadTDO:%3d\n", tms, tdi, read_tdo);

	if (!jtag_hardware_initialized)
	{
	        fprintf(stderr, "Info:  jtag_hardware_initialized is false, calling init function\n");
		//while(1);
	...
	{
	        //fprintf(stderr, "Info:  specified_com_port is true\n");
		//ch_data = 'x';

		ch_data = (unsigned char)( (read_tdo ? 0x10 : 0) | (tdi ? 0x04 : 0) | (tms ? 0x80 : 0)) | 0x01; // serial interface will only transmit of ch_data != 0x00??
		write(com_port, &ch_data, 1);
		//fprintf(stderr, "Info:  send char 1: 0x%02X\n", ch_data);
		//fprintf(stderr, "Info:  send char: 0x%02X\n", ch_data);

		// busy-waiting for response
		while(read(com_port, &c, 1) <= 0)
		{
		}

		if (read_tdo)
		{
			//fprintf(stderr, "Response: 0x%02X, ", c);
			tdo = (c & 0x08) ? 1 : 0;
			//fprintf(stderr, "Info:  Read TDO: %d\n", tdo);
		}

		// no further data from serial interface expected, what if?
		if(read(com_port, &c, 1) > 0)
		{
			fprintf(stderr, "Communication Error.\r\n");
		}

		ch_data_m = ch_data | 0x02; // TCK high
		write(com_port, &ch_data_m, 1);
		//fprintf(stderr, "Info:  send char 2: 0x%02X\n", ch_data_m);
		...
		write(com_port, &ch_data, 1);
		//fprintf(stderr, "Info:  send char 3: 0x%02X\n", ch_data);
	...
	{
		fprintf(stderr, "Error:  specified_com_port is false\n");
	}
...
void jam_delay(long microseconds)
{
	...
	// ported to Linux/Unix by Krafft/Weber
	usleep(microseconds);
}
...
DWORD get_tick_count(void)
{
	...
	tick_count = (DWORD) (clock() / 1000L ); // Weber/Krafft: function not really used
	//printf("get_tick_count(void) called\n");
#endif
...
void calibrate_delay(void)
{
	...
	// no customization needed, as this function is not used (Krafft/Weber)
#endif
}
...
/*
		...
		//tty_fd=open(serial_port_name, O_RDWR | O_NONBLOCK);      
*/

		com_port = open(serial_port_name, O_RDWR | O_NONBLOCK);
		if (com_port == -1)
		{
		...
		else
		{
			...
			fprintf(stderr, "Info: serial port \"%s\" opened\n", serial_port_name);
		}
	}
}
